/**
 * Ping / System Diagnostics — Discord version
 * -----------------------------
 * Dikonversi dari ping.js (WA, pakai sock.sendMessage) ke Discord.js.
 * Logic canvas-nya PERSIS sama seperti versi asli (udah pakai
 * @napi-rs/canvas yang emang dependency bot ini) — cuma bagian
 * pengiriman pesannya yang diganti ke AttachmentBuilder Discord.
 */
...
export const config = {
    ...
    category: 'general',
    description: 'Cek performa dan status sistem bot secara real-time (Canvas)',
    usage: '.ping',
    example: '.ping',
    isOwner: false,
    isPremium: false,
...
async function createPingCanvas(data) {
    const width = 1200;
    const height = 800;
    ...
    ctx.fillText(`STATUS: ONLINE | TERMINAL: DISCORD.JS`, 50, 110);
...
export async function handler(m) {
    try {
        ...
        const imageBuffer = await createPingCanvas(data);
        const attachment = new AttachmentBuilder(imageBuffer, { name: 'ping.png' });
        ...
        const caption =
            ...
            `> ◦ NodeJS: ${data.nodeVer}\n\n` +
            `💻 **CPU**\n` +
            `> ◦ ${data.cpuModel.trim()} — ${data.cpuCores} Core(s)\n\n` +
            `🧠 **MEMORI**\n` +
            `> ◦ Dipakai: ${fmtSize(data.usedMem)} / ${fmtSize(data.totalMem)} (${data.memPct}%)\n\n` +
            `⏱️ **UPTIME**\n` +
            `> ◦ Server: ${data.upOS} | Bot: ${data.upBot}`;
